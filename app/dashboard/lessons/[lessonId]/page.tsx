...
function AudioExercisesPanel({ exercises, vocabulary }: { exercises: AudioExercise[]; vocabulary: VocabularyItem[] }) {
  ...
  const getAudioSrcForText = (text: string, language: 'hausa' | 'arabic') => {
  const item = vocabulary.find((item) => item.hausa === text || item.arabic === text);

  if (!item) return undefined;

  return language === 'hausa' ? item.hausaAudioSrc : item.arabicAudioSrc;
  };
  ...
  playAudio(
    ...
    exercise.targetLanguage === 'hausa'
      ? exercise.hausaAudioSrc
      : exercise.arabicAudioSrc
  )
}
  ...
  text={pair.left}
  language="hausa"
  audioSrc={getAudioSrcForText(pair.left, 'hausa')}
  label="Hausa"
  size="sm"
/>
                      </button>
